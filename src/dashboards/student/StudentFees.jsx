...
const StudentFees = () => {
  ...
  const fetchFees = async () => {
    try {
      setLoading(true);
      const response = feesAPI.paymentHistory();
      setPayments(response.data || []);
    } catch (error) {
      console.error('could not fetch payment', error)
      ...
      >
        <div>
          ...
          <p className="text-gray-600">
            View and manage your fee invoices and payments
          </p>
        </div>
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-sm text-gray-500">
                        <span
                          className={`px-2 py-1 rounded-full text-xs ${getCategoryColor(invoice.category)}`}
                        >
                          {invoice.category}
                        </span>
                      </div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        {getStatusIcon(invoice.status)}
                        <span
                          className={`ml-2 px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(invoice.status)}`}
                        >
                          {invoice.status}
                        </span>
                      </div>
        ...
        <div className="p-6">
          ...
          <div className="mb-8 p-6 bg-gradient-to-r from-primary-50 to-primary-100 rounded-xl">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <span
                  className={`px-4 py-2 rounded-full text-sm font-medium ${getStatusColor(invoice.status)}`}
                >
