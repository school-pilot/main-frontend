...
import { teachersAPI, academicsAPI } from '../../services/api';
...
const CreateTeacher = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (user && !user.is_active) {
      toast.error('Your account is inactive. Cannot create teacher.');
      return;
    }
    ...
    try {
      // Create teacher with teacher role automatically
      const teacherData = {
        ...formData,
    ...
    <Paper sx={{ p: 3, maxWidth: 800, mx: 'auto' }}>
      ...
      {user && !user.is_active && (
        <Alert severity="warning" sx={{ mb: 3 }}>
          Your account is inactive. You cannot create teachers until activated by super admin.
        </Alert>
      )}
      ...
      <Box component="form" onSubmit={handleSubmit}>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <Autocomplete
              multiple
              options={availableSubjects.map(subject => subject.name)}
